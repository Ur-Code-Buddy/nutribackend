...
async function run() {
  ...
  try {
    ...
    await axios.post(
      `${api}/admin/users/${actualUserId}/disable`,
      {},
      { headers: { Authorization: `Bearer ${adminToken}` } },
    );
    ...
    const userToPrint = res.data.find(
      (u) => u.username === userPayload.username,
    );
    if (userToPrint) {
      console.log(
        `Normal User details from Admin view: ID=${userToPrint.id}, Name=${userToPrint.name}, Credits=${userToPrint.credits}`,
      );
    } else {
