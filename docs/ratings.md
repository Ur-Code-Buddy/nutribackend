...
There is **no** `POST /reviews` endpoint. Clients submit stars via **`POST /orders/:orderId/items/:itemId/rating`** (`:itemId` = `order_item_id`).
...
**Response shapes:** **`GET /reviews/my`** and **`GET /reviews/kitchen/:kitchenId`** return a **JSON array** of review rows. **`GET /reviews/food-item/:foodItemId`** returns a **JSON object** with **`reviews`** plus order-volume fields (see below).
...
| Field                      | Type   | Description                                                                                                                                 |
| -------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `reviews`                  | array  | **Review** rows for that food item, **`created_at`** descending (same fields as stored: ids, **`stars`**, timestamps, foreign keys, etc.). |
| `total_orders`             | number | Distinct orders that include this food item, excluding **REJECTED** orders (aligned with capacity / sold counts elsewhere).                   |
| `total_quantity_ordered`   | number | Sum of **`order_items.quantity`** for those lines — total units ordered (e.g. two per line counts as two toward this total).                 |
...
| `average_rating`          | number \| null | Arithmetic mean of **`reviews.stars`** for items in this kitchen (two decimals); **`null`** if no ratings      |
...
- `src/reviews/reviews.service.ts` — upsert, listings (food-item payload includes order counts), stars map for orders
