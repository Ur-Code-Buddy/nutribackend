...
async function checkS3() {
    ...
    try {
        ...
        if (!data.Buckets?.some(b => b.Name === process.env.AWS_BUCKET_NAME)) {
            console.warn(`⚠️ Warning: Configured bucket '${process.env.AWS_BUCKET_NAME}' not found in account.`);
        } else {
            console.log(`✅ Configured bucket '${process.env.AWS_BUCKET_NAME}' exists.`);
        }
    } catch (err) {
